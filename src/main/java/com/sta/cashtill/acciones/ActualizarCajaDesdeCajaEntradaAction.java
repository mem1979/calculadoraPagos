...
public class ActualizarCajaDesdeCajaEntradaAction extends SaveAction {
    ...
    public void execute() throws Exception {
     ...
     // Validar que haya detalles para procesar
        ...
        if ((cajaEntrada.getVuelto().compareTo(BigDecimal.ZERO) <= 0) || (getView().getValue("conVuelto").equals(true))) {
     	...
     	MovimientoCaja movimiento = MovimientoCaja.findByNombre("VUELTO");
        ...
        getView().setValueNotifying("movimientoCaja.id",movimiento.getId()); // Asignar el ID de "VUELTO"
        getView().setEditable("movimientoCaja", false);
        
        getView().setValue("importe", cajaEntrada.getVuelto());
        getView().setEditable("importe", false);
        
        getView().setValue("estrategiaPagos", EstrategiaPagos.MEJOR_AJUSTE);
