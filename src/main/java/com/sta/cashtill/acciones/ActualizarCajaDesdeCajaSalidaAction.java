    ...
    public void execute() throws Exception {
        ...
        if (getErrors().isEmpty()) {
            for (DetalleCajaRegistradora detalle : cajaSalida.getDetalle()) {
                ...
                Caja caja = buscarCajaPorId(detalle.getCajaId());
                if (caja == null) {
                    addError("No se encontró una caja con el ID: " + detalle.getCajaId());
                    continue;
                }
    ...
    private boolean validarCajas(CajaSalida cajaSalida) {
        for (DetalleCajaRegistradora detalle : cajaSalida.getDetalle()) {
            ...
            if (caja == null || caja.getCantidad() == 0 || caja.getCantidad() < detalle.getCantidad()) {
                addError("La caja con ID " + detalle.getCajaId() + " no tiene suficiente cantidad.");
                return false;
            }
        }
        return true;
